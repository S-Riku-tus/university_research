"""Freeze and analyze the completed September 15, 3 kHz, seven-noise run.
...
def main():
    ...
    paths = [p for p in base.glob("maxfreq=3kHz/*/*/run_manifest.json")
             if json.loads(p.read_text(encoding="utf-8-sig"))["run_hash"] == "6f5512"]
    ...
    tuning = [r for r in tuning if r["max_freq_hz"] == "maxfreq=3kHz" and r["run_hash"] == "6f5512"]
    ...
    fig.suptitle("2025-06-11 / 3 kHz / matched training / 18 source WAVs / seed 42")
    ...
    for ax, mat, label in zip(axes, matrix_list, LABELS[:3]):
        ...
        ax.set_yticks(range(len(freq_order)), [n.replace("freq_", "").replace("Hz", "") for n in freq_order])
    ...
    manifest = {"collected_at": datetime.now().astimezone().isoformat(), "scope": RUN+"/maxfreq=3kHz",
                "run_hash": "6f5512", "sources": sources, "counts": {k:len(v) for k,v in tables.items()},
                ...
                "notes": ["Source files and ongoing other-frequency runs were not modified.",
                          "Training summary file is shared with later runs; extracted 3 kHz rows are frozen here.",

...
4. それでもGPU上の未実装演算が生じた場合だけ、同じIGをCPUで自動再試行する。
...
通常は決定論GPU上の非fused BatchNormalizationを使う。CPU fallbackは未実装GPU演算が残る場合だけ使用する。
...
- `tests/test_integrated_gradients.py`: 10件成功。既存の解析解、raw/log-power経路、符号、completeness、保存診断、stability・sanity checkを維持した。
- 全テスト: 75件成功。

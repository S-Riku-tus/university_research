...
## inner holdoutの内部制約と別日一般化の前提

全noise共通重みの問題は解消済みだが、現在の`inner_holdout`には次の内部制約が残る。ここで、現行設定の3-foldは深層モデルのepoch選択用であり、重み用holdoutを3回繰り返す設定ではない。別日評価では外側foldも1回であるため、重みは各seed・各noiseにつき18 WAV中4 WAVのholdout 1回から求める。
...
これとは別に、「同じ実験条件なら学習日内の性能関係が別の評価日にも一定程度一般化する」という前提は、`inner_holdout`固有の欠点ではなく、別日train/testを行う本研究全体に必要な前提である。名目上同じ条件でも日差があるため、その成立度を評価日で確認する。今回noiseあり18条件では、最大重みモデルと6/18の最良単体が一致したのは18条件中6条件、重み順位と6/18性能順位の平均Spearman相関は0であり、この日対では順位移送が弱かったという確認事実に留める。
...
5. その結果でも単体R²重みの限界が残る場合、同じOOFを共有して結合規則だけを変える設計へ整理したうえで、`subset_equal_cv`と`crossfit_shrinkage_stack`を比較する。
6. 方式を一つに固定した後、学習日を6/11+6/18、評価日を7/9へ変更し、同じコード・同じ条件で別日確認する。最終候補runでは保存モデルと再読込検証を有効にする。

次条件は[performance_kfold主方式・固定200 epoch比較](../2026-09-25_matched_performance_kfold/README.md)に記録した。

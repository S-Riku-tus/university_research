...
def vgg16_regression(input_shape):
    ...
    model.add(Conv2D(256, (3, 3), activation='relu', padding='same'))
    model.add(Conv2D(512, (3, 3), activation='relu', padding='same'))  # 普通は(1, 1)のカーネルサイズで畳み込む
    ...
    model.add(Conv2D(512, (3, 3), activation='relu', padding='same'))  # 普通は(1, 1)のカーネルサイズで畳み込む
    # model.add(Conv2D(512, (1, 1), activation='relu', padding='same'))
    model.add(MaxPooling2D((2, 2), strides=(2, 2)))  # 普通は(7, 7)のカーネルサイズでダウンサンプリングする
        ...
        for batch_size in BATCH_SIZES:
            for epoch_num in EPOCH_NUMS:
                ...
                for train_index, val_index in kf.split(x):
                    ...
                    # AlexNetモデルの作成とコンパイル
                    model = vgg16_regression(input_shape=(224, 224, COLOR_CHANNEL))

...
> **9/15追加比較後の更新先**: RFを固定の基準モデルとする提案や追加モデル/stackingは当時の候補。現行innerは元WAV非重複へ修正済み。今回はIGが両帯域で不整合であり、物理的補完の確定や新方式の採用を意味しない。 最新の[比較解析](../../experiments/2026-09-15_onb_frequency_comparison/analysis.md)、[本人の研究仮説](2026-09-15_master_thesis_hypothesis.md)、[解析手順](../analysis_workflow.md)を参照する。以下の本文は当時の記録として保持する。

> **履歴・適用範囲（2026-09-15追記）**: この文書は当時の条件・判断の記録です。本文の「現在」「次にやること」は作成時点を指します。最新の設定・完了範囲・優先順位は[研究の現在地](../research_status.md)、更新関係は[文書案内](../document_index.md)を参照してください。 RF優位・全モデル2–5 kHz等の当時の前提と将来案を、現行log-powerモデルの確定結果や本人の確定方針として扱わないでください。

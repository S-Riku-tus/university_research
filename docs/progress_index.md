...
- [追跡監査](../experiments/2026-09-24_selection_onb_ig_review/README.md)で、原記録ONBと現行コード値が3実験とも不一致と確認。Cの除外94秒は原ONBでは全てONB前で、意図したONB以降選別になっていなかった。
- 全2,940秒の2.1–2.5 kHzピークを原ONBで再集計。単一絶対閾値は日間移植性がなく、6/11学習だけの識別候補を選別なし、`3e-8`、`1e-7`と同数ランダム除外に限定した。
...
- 追加実験と同期映像は取得不可。未知実験日への一般化と映像同期による物理同定は今後の主張範囲から外す。
...
- Cの除外94秒を全件監査。221,505 W/m²で60秒、266,908で25秒、315,466で9秒を除外し、原記録のONB 369,000 W/m²では全てONB前となる。追加取得・同期映像・独立ノイズを使わない前提では、選別不採用と閾値依存・交絡を限界として閉じる。
...
- 原テキストのONB開始熱流束とrun閾値は3実験日で異なる。原値の解釈と今後の独立日・音声同期気泡映像の取得可否は教授へ確認する。本人確認では追加の未使用実験日と同期映像は現時点でない。

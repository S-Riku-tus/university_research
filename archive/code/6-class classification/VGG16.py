...
# 特徴量データの読み込み
TRAIN_PATH = r"C:\Users\Casper3\Python\Ueki\shibasaki\研究\6月26日_サブクール度10度_0.3mm\スペクトログラム"

# val-lossグラフの保存先フォルダ
SAVE_PATH = r"C:\Users\Casper3\Python\Ueki\shibasaki\研究\6月26日_サブクール度10度_0.3mm\training_results\6クラス分類\VGG16"
...
def load_images_from_folder(folder_path):
    ...
    min_images = min(len(image_files[class_name]) for class_name in class_names[:-1])  # not_boiling を除外して最小値を取得

    # not_boiling は min_images になるようにランダムにサンプル
...
# VGG16の読み込み（重みはImageNetなら='imagenet'、初期化するなら=None）
def vgg16(input_shape, num_classes):
    base_model = VGG16(weights=None, include_top=False, input_shape=input_shape)

    # ベースモデルに追加するレイヤーを定義
    x = base_model.output
    x = GlobalAveragePooling2D()(x)  # GlobalAveragePooling2Dレイヤーを追加
    x = Dense(4096, activation='relu')(x)  # Dense層を追加
    x = Dropout(0.5)(x)  # ドロップアウト層を追加
    x = Dense(4096, activation='relu')(x)  # 追加
    x = Dropout(0.5)(x)  # ドロップアウト層を追加
    predictions = Dense(num_classes, activation='softmax')(x)  # 分類層を追加
...
# VGG16モデルの構築
model = vgg16(input_shape=(224, 224, 3), num_classes=6)

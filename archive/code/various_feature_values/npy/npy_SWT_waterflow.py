...
# 生成された画像を保存するフォルダのパス
base_save_folder_path = r"C:\Users\Casper4\Python\ueki\shibasaki\研究\Pool_boiling\Subcooling_20_degrees\0.3\2024.11.12_1_2.13_1\data\CWT\npy\waterflow_" + f"{SAVE_DATE}"
...
def save_spectrogram_chunks_with_snr(file_path, waterflow_path, base_save_folder_path, max_freq_hz, one_chunk=1, snr_db=SNR_list):
    ...
    for snr in snr_db:
        ...
        snr_save_folder_path = os.path.join(base_save_folder_path, f"heatflux_{snr_label}") # パス生成方法を変更
        ...
        for i in range(0, len(y_noisy), data_length):
            ...
            # scale
            scale = calc_scale(frequency, sr, 1)

            swt = calc_swt(chunk, frequency, scale, sr)

            # ★最大周波数までのデータで切り取る★
            # 計算した max_k を使用
            swt_cropped = swt[:, :max_k + 1]

            # ★切り取ったデータをリサイズ★
            resized_amplitude = resize(swt_cropped, (224, 224))
            # ★ここまで修正★
            ...
            plt.imshow(np.rot90(resized_amplitude, k=2), cmap='jet', aspect='auto', origin='lower',
            ...
            save_path = os.path.join(snr_save_folder_path, f"{extracted_label}_{i//data_length}.png")
...
for filename in os.listdir(folder_path):
    if filename.endswith(".wav"):
        ...
        save_spectrogram_chunks_with_snr(file_path, waterflow_path, base_save_folder_path, max_freq_hz,
                                         one_chunk=1, snr_db=SNR_list) # ★ここで max_freq_hz を渡す★

...
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, r2_score, roc_curve, auc
...
BATCH_SIZES = {"AlexNet": 12, "ResNet50": 16, "VGG16": 32}
BATCH_SIZES_ALL = [4, 12, 24]
BATCH_SIZES_ALL = [4]
EPOCH_NUM = 100
# 学習率
# 今のところ一番良いやつ
LEARNING_RATE = {"AlexNet": 0.05, "ResNet50": 0.001, "VGG16": 0.005}
LEARNING_RATE_ALL = [0.005, 0.001, 0.0005, 0.0001, 0.00005]
LEARNING_RATE_ALL = [0.005]


for all_bs in BATCH_SIZES_ALL:
    for all_lr in LEARNING_RATE_ALL:
        LEARNING_RATE = {"AlexNet": 0.0001, "ResNet50": 0.0001, "VGG16": 0.0005}
        BATCH_SIZES = {"AlexNet": 32, "ResNet50": 48, "VGG16": 32}
        ...
        if ENSEMBLE_METHOD == 0:
            SAVE_PATH = os.path.join(BASE_SAVE_PATH, f"{SAVE_DATE}_ep{EPOCH_NUM}_chu{CHUNK}", f"average")
            ...
            SAVE_PATH = os.path.join(BASE_SAVE_PATH, f"SGD_{SAVE_DATE}_ep{EPOCH_NUM}_bsAl{bsare}_Re{bsres}_Vg{bsvgg}_lrAl{lrale}_Re{lrres}_Vg{lrvgg}")
        elif ENSEMBLE_METHOD == 2:
            SAVE_PATH = os.path.join(BASE_SAVE_PATH, f"{SAVE_DATE}_ep{EPOCH_NUM}_chu{CHUNK}", f"min")
            ...
            for data_path in DATA_PATH:
                ...
                with open(output_file, 'a', encoding='utf-8') as f:
                    ...
                    for train_index, val_index in kf.split(x):
                        ...
                        alexnet_model = regressionmodelmaker.cnn_transformer()
                        resnet50_model = regressionmodelmaker.cnn_transformer()
                        vgg16_model = regressionmodelmaker.cnn_transformer()
                        ...
                        alexnet_model.compile(optimizer=SGD(learning_rate=LEARNING_RATE['AlexNet'], momentum=0.7, clipnorm=1.0), loss='mean_squared_error')
                        ...
                        vgg16_model.compile(optimizer=SGD(learning_rate=LEARNING_RATE['VGG16'], momentum=0.7, clipnorm=1.0), loss='mean_squared_error')
